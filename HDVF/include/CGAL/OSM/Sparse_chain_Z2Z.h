...
/*!
 ...
 The class `Sparse_chain` implements the concept `SparseChain`, that is, sparse vectors (encoding homological chains)
 optimized for topological computations. Given a complex (for instance a simplicial, cubical or cellular complex) which
 cells in a given dimension \f$q\f$ are numbered
 \f$\{\sigma_i\,;\, i=1\ldots n_q\}\f$, homology with coefficients in a given ring \f$\mathbb K\f$ defines
 \f$q\f$-chains as formal linear combinations:
 \f\[\gamma = \sum_{i=1}^{n_q}\lambda_i\cdot \sigma_i\f\]
 with coefficients \f$\lambda_i\in\mathbb K\f$.
 In the basis \f$\{\sigma_i\,;\, i=1\ldots n_q\}\f$, the coordinates of \f$\gamma\f$ are given by the vector:
 \f$[\lambda_1,\ldots \lambda_{n_q}]\f$.
 ...
 Moreover, as per any linear algebra vector, a `Sparse_chain` is either a column or row vector (the `StorageFormat`
 parameter determines this storage format).

 The class `Sparse_chain` provides standard linear algebra operators and fast iterators and block operations (set, get
 and nullify) which are required to implement efficiently HDVFs.
 ...
 homology.
 \tparam StorageFormat an integer constant encoding the storage format of matrices (`CGAL::OSM::COLUMN` or
 `CGAL::OSM::ROW`).
*/

template <int StorageFormat> class Sparse_chain_z2z
{
...
public:
  ...
  template <int _CTF> friend class Sparse_chain_z2z; // TODO : CHECK if we need to add Sparse_chain classic as a friend
...
public:
   ...
   *
   * Constructor with size, initializes an empty sparse chain encoding a linear combination of cells with all
   * coefficients null.
   *
   ...
   */
  Sparse_chain_z2z(const Sparse_chain_z2z& otherToCopy)
   ...
   */
  Sparse_chain_z2z& operator=(const Sparse_chain_z2z& otherToCopy) {
    _upperBound = otherToCopy._upperBound;
    _chainData = otherToCopy._chainData;
   ...
   */
  friend std::ostream& operator<<(std::ostream& stream, const Sparse_chain_z2z& chain) {
    stream << "[";
   ...
   */
  template <int _SF> friend std::istream& read_chain(Sparse_chain_z2z<_SF>& chain, std::istream& in);

  /** \relates Sparse_chain
   *
   * \brief Reads a sparse chain from a file.
   */
  template <int _SF> friend void read_chain(Sparse_chain_z2z<_SF>& chain, std::string filename);
   ...
   */
  Sparse_chain_z2z operator+(const Sparse_chain_z2z& other) {
    Sparse_chain_z2z newChain = *this;
    newChain += other;
  ...
  */
   ...
   * \return A new chain representing the result.
   */
  Sparse_chain_z2z operator*(const CoefficientRing& lambda) {
    Sparse_chain_z2z newChain = *this;
    newChain *= lambda;
   ...
   */
  friend CGAL::Z2 operator*(const Sparse_chain_z2z<ROW>& row, const Sparse_chain_z2z<COLUMN>& column);
  ...
  /**
   * \brief Sets a coefficient.
   *
   * Equivalent to set_coefficient. A value of 0 is equivalent to erase.
   *
   * \param index The coefficient index.
   * \param value The value to set (0 or 1).
   */
  inline void set(size_t index, Coefficient_ring value) { set_coefficient(index, value); }

  /**
   * \brief Gets a coefficient value.
   *
   * \param index The coefficient index.
   *
   * \return The coefficient value at given index.
   */
  inline Coefficient_ring get(size_t index) const { return get_coefficient(index); }

  /**
   * \brief Adds another chain to this chain in Z/2Z in place.
   *
   * Performs coefficient-wise XOR addition. Coefficients that become 0 are removed
   * to preserve the sparse representation.
   *
   * \pre Chains must have the same size.
   *
   * \param other The other chain to add.
   *
   * \return A reference to the modified chain.
   */
  Sparse_chain_z2z& add(const Sparse_chain_z2z& other) { return *this += other; }

  /**
   * \brief Erases the coefficient at the given index.
   *
   * Sets the coefficient at `index` to 0 and removes it from the sparse storage.
   * Equivalent to set(index, 0).
   *
   * \param index The index of the coefficient to erase.
   */
  inline void erase(size_t index) { *this /= index; }
   ...
   */
  const bool is_null() const { return _chainData.size() == 0; }
   ...
   * \return A new chain representing the result.
   */
  Sparse_chain_z2z operator/(const std::vector<size_t>& indices) {
    Sparse_chain_z2z newChain = *this;
    newChain /= indices;
    return newChain;
  }
   ...
   */
  Sparse_chain_z2z operator/(size_t index) {
    Sparse_chain_z2z newChain = *this;
    newChain /= index;
    return newChain;
  }
   ...
   * \return Return a reference to the modified chain.
   */
  Sparse_chain_z2z& operator/=(const size_t index) {
    this->_chainData.erase(index);
   ...
   */
  void nullify() { this->_chainData.clear(); }
   ...
   */
  iterator begin() noexcept { return _chainData.begin(); }
   ...
   */
  const_iterator begin() const noexcept { return _chainData.begin(); }
   ...
   * \return The function returns a constant iterator to the first non zero index.
   */
  const_iterator cbegin() const noexcept { return _chainData.cbegin(); }
   ...
   */
  iterator end() noexcept { return _chainData.end(); }
   ...
   */
  const_iterator end() const noexcept { return _chainData.end(); }
   ...
   */
  const_iterator cend() const noexcept { return _chainData.cend(); }
   ...
   */
  bool is_column() const { return StorageFormat == COLUMN; }
   ...
   */
  bool is_row() const { return StorageFormat == ROW; }
