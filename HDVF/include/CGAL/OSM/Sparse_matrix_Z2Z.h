...
// All rights reserved.
//
// This file is part of CGAL (www.cgal.com)
//
// $URL$
// $Id$
...
// 		   Dalil Naamna <dalil.naamna@etu.univ-amu.fr>
// 		   Léandre Gély <gely.leandre@gmail.com>

#ifndef CGAL_OSM_SPARSE_MATRIX_Z2Z_H
#define CGAL_OSM_SPARSE_MATRIX_Z2Z_H
...
/*!
 ...
 The class `Sparse_matrix_z2z` implements the concept `SparseMatrix`, that is, sparse matrices optimized for topological
 computations. It provides standard linear algebra operators and fast iterators and block operations (set, get and
 nullify) which are required to implement efficiently HDVFs.

 The implementation is based on mapped sparse matrices. Hence matrices of the `Sparse_matrix_z2z` class are either
 column of row major (the `StorageFormat` parameter determines this storage format). A column-major (resp. row-major)
 `Sparse_matrix` is a vector of `Sparse_chain` which encode columns (res. rows). Moreover, in order to efficiently
 iterate over non empty columns (resp. rows) the `Bitboard` data structure implements the concept
 `SparseMatrix::NonZeroChainIndices`. A bitboard is basically a bucket of bits recording the indices of non empty
 chains. However, this data structure has been designed in order to efficiently remove or add indices, as well as
 provide efficient iterators to visit non empty chains.
 ...
 Figures below shows the data structures are created according to the chosen representation  (left: column-major, right:
 row-major):

 <img src="Sparse_matrix_z2z_example_col.pdf" align="center" width=20%/>
 <img src="Sparse_matrix_z2z_example_row.pdf" align="center" width=20%/>

 \cgalModels{SparseMatrix}

 \tparam CoefficientRing a model of the `IntegralDomainWithoutDivision` concept, providing the ring used to compute
 homology.
 \tparam StorageFormat an integer constant encoding the storage format of matrices (`OSM::COLUMN` or `OSM::ROW`).
 */

template <int StorageFormat> class Sparse_matrix_z2z
{
...
public:
   ...
   */
  typedef CGAL::Z2 Coefficient_ring; // TODO : To Be Checked
  typedef CGAL::Z2 CoefficientRing;  // TO not have to replace it every wherer // TODO : To Be Checked
...
protected:
  ...
  std::vector<Sparse_chain_z2z<StorageFormat>> _chains; // TODO : To Be Checked
...
public:
   ...
   *
   * Create a new empty `Sparse_matrix_z2z` object of type `StorageFormat` with coefficients of type `CoefficientRing`
   * and a given size along rows/columns.
   *
   ...
   */
  Sparse_matrix_z2z(const std::pair<size_t, size_t> dimensions)
      : Sparse_matrix_z2z(dimensions.first, dimensions.second) {}

  /**
   * \brief Copy constructor.
   *
   * Create a new SparseMatrix from another SparseMatrix object (with possibly a different `StorageFormat`). Initialize
   * a SparseMatrix of same sizes, containing the same coefficients (but not necessarly of the same `StorageFormat`). If
   * types are different, the constructor performs conversion.
   *
   * \param otherToCopy The matrix copied.
   */
  template <int CTF> Sparse_matrix_z2z(const Sparse_matrix_z2z<CTF>& otherToCopy) {
    if(StorageFormat == CTF) {
      ...
      for(size_t i = 0; i < otherToCopy._chains.size(); ++i) {
        ...
        Sparse_chain_z2z<StorageFormat> res(tmp.dimension());
  ...
  void eye(size_t nrows, size_t ncols) {
    *this = Sparse_matrix_z2z(nrows, ncols);
   ...
   */
  void eye(std::pair<size_t, size_t> dimensions) { eye(dimensions.first, dimensions.second); }
   ...
   */
  Sparse_matrix_z2z& operator=(const Sparse_matrix_z2z& otherToCopy) {
    _chainsStates = otherToCopy._chainsStates;
    _size = otherToCopy._size;
   ...
   */
  bool is_null() { return (_chainsStates.begin() == _chainsStates.end()); }

  /**
   * \brief Tests if a SparseMatrix is empty.
   *
   * The function return `true` is the SparseMatrix is empty (that is, if the number of columns or rows is null) and
   * `false` otherwise.
   */
  bool is_empty() { return ((_size.first == 0) || (_size.second == 0)); }
   ...
   */
  friend std::ostream& operator<<(std::ostream& stream, const Sparse_matrix_z2z& matrix) {
#ifndef DEBUG
    bool empty = true;
   ...
   */
  friend std::ostream& write_matrix(const Sparse_matrix_z2z<OSM::COLUMN>& M, std::ostream& out);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Writes a sparse ROW matrix to a stream.
   */
  friend std::ostream& write_matrix(const Sparse_matrix_z2z<OSM::ROW>& M, std::ostream& out);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Writes a sparse COLUMN matrix to a file.
   */
  friend void write_matrix(const Sparse_matrix_z2z<OSM::COLUMN>& M, std::string filename);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Writes a sparse ROW matrix to a file.
   */
  friend void write_matrix(const Sparse_matrix_z2z<OSM::ROW>& M, std::string filename);
   ...
   */
  friend std::istream& read_matrix(Sparse_matrix_z2z<OSM::COLUMN>& M, std::istream& in);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Reads a sparse ROW matrix from a stream.
   */
  friend std::istream& read_matrix(Sparse_matrix_z2z<OSM::ROW>& M, std::istream& in);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Reads a sparse COLUMN matrix from a file.
   */
  friend void read_matrix(Sparse_matrix_z2z<OSM::COLUMN>& M, std::string filename);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Reads a sparse ROW matrix from a file.
   */
  friend void read_matrix(Sparse_matrix_z2z<OSM::ROW>& M, std::string filename);
   ...
   *
   * Adds each coefficient of the matrices and returns a new matrix (of the same type as `this`) representing the result
   * (when possible, prefer `+=` for efficiency).
   *
   ...
   */
  template <int _CTF> Sparse_matrix_z2z operator+(const Sparse_matrix_z2z<_CTF>& other) {
    Sparse_matrix_z2z newMatrix(*this);
    newMatrix += other;
   ...
   *
   * Subtracts each coefficient of the matrix `other` and returns a new matrix (of the same type as `this`) representing
   * the result (when possible, prefer `-=` for efficiency).
   *
   * \pre Matrices must have the same `CoefficientRing` but can have different `StorageFormat`.
   *
   * \warning Will raise an error if the other matrix is not the same `CoefficientRing`.
   ...
   */
  template <int _CTF>
  friend Sparse_matrix_z2z operator*(const CGAL::Z2& lambda, const Sparse_matrix_z2z<_CTF>& matrix) {
    Sparse_matrix_z2z newMatrix = matrix;
    newMatrix *= lambda;
   ...
   * \return A new matrix representing the result.
   */
  Sparse_matrix_z2z operator*(const CoefficientRing& lambda) {
    Sparse_matrix_z2z newMatrix = *this;
    newMatrix *= lambda;
   ...
   *
   * Perform standard linear algebra multiplication of matrices and return a new column-major matrix (when possible,
   * prefer `*=` for efficiency). Matrices must have the same `CoefficientRing` but can have different `StorageFormat`.
   * The multiplication is optimized (using standard definition or block multiplications) for each combination of
   * `StorageFormat`. However, efficiency depends on `StorageFormat` (when possible, prefer row-major by column-major
   * multiplications).
   *
   ...
   */
  friend Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& first,
                                             const Sparse_matrix_z2z<COLUMN>& second);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Matrix multiplication: ROW x COLUMN -> COLUMN
   */
  friend Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& first,
                                             const Sparse_matrix_z2z<COLUMN>& second);
   ...
   */
  friend Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& first,
                                             const Sparse_matrix_z2z<ROW>& second);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Matrix multiplication: ROW x ROW -> COLUMN
   */
  friend Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& first, const Sparse_matrix_z2z<ROW>& second);
   ...
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief  Perform multiplication of a sparse matrix (column or row major) and a column-chain. The function returns a
   * new column-major chain.
   *
   * Perform standard linear algebra multiplication of a matrix and a column-chain (ie.\ matrix / column vector
   * multiplication) and returns a new column-major chain. Both arguments must have the same `CoefficientRing` but the
   * matrix can have any `StorageFormat` (and the multiplication is optimized for each of them).
   *
   * @param matrix The matrix.
   ...
   */
  friend Sparse_chain_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& _matrix,
                                            const Sparse_chain_z2z<COLUMN>& _column);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Matrix/column chain multiplication: ROW matrix x COLUMN chain -> COLUMN chain.
   */
  friend Sparse_chain_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& _matrix,
                                            const Sparse_chain_z2z<COLUMN>& _column);
   ...
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief  Perform multiplication of a row-chain and a sparse matrix (column or row major). The function returns a new
   * row-major chain.
   *
   * Perform standard linear algebra multiplication of a row-chain and a matrix (ie.\ row vector / matrix
   * multiplication) and returns a new row-major chain. Both arguments must have the same `CoefficientRing` but the
   * matrix can have any `StorageFormat` (and the multiplication is optimized for each of them).
   *
   ...
   */
  friend Sparse_chain_z2z<ROW> operator*(const Sparse_chain_z2z<ROW>& _row, const Sparse_matrix_z2z<ROW>& _matrix);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Row chain/matrix multiplication: ROW chain x ROW matrix -> ROW chain.
   */
  friend Sparse_chain_z2z<ROW> operator*(const Sparse_chain_z2z<ROW>& _row, const Sparse_matrix_z2z<COLUMN>& _matrix);
   ...
   *
   * Perform standard linear algebra multiplication of matrices and return a new row-major matrix (when possible, prefer
   * `*=` for efficiency). Matrices must have the same `CoefficientRing` but can have different `StorageFormat`. The
   * multiplication is optimized (using standard definition or block multiplications) for each combination of
   * `StorageFormat`. However, efficiency depends on `StorageFormat`.
   *
   * @param first The first matrix.
   * @param second The second matrix.
   * @return The result of the matrix multiplication, row-based.
   ...
   */
  friend Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<COLUMN>& _first,
                                          const Sparse_matrix_z2z<COLUMN>& _second);

  /** \relates Sparse_matrix_z2z
   *
   * \brief matrix multiplication: ROW x COLUMN -> ROW
   */
  friend Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<ROW>& _first,
                                          const Sparse_matrix_z2z<COLUMN>& _second);
   ...
   */
  friend Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<COLUMN>& _first,
                                          const Sparse_matrix_z2z<ROW>& _second);

  /** \relates Sparse_matrix_z2z
   *
   * \brief matrix multiplication: ROW x ROW -> ROW
   */
  friend Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<ROW>& _first, const Sparse_matrix_z2z<ROW>& _second);
   ...
   *
   * Perform standard linear algebra addition. Matrices must have the same `CoefficientRing` but can have different
   * `StorageFormat`. The addition is optimized for each combination of `StorageFormat`.
   *
   * @param matrix The first matrix.
   * @param other The second matrix.
   ...
   */
  Sparse_matrix_z2z& operator+=(const Sparse_matrix_z2z& other) {
   ...
   */
  friend Sparse_matrix_z2z<COLUMN>& operator+=(Sparse_matrix_z2z<COLUMN>& matrix, const Sparse_matrix_z2z<ROW>& other);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Matrices sum and assign: ROW += COLUMN.
   */
  friend Sparse_matrix_z2z<ROW>& operator+=(Sparse_matrix_z2z<ROW>& matrix, const Sparse_matrix_z2z<COLUMN>& other);
   ...
   *
   * Perform standard linear algebra subtraction. Matrices must have the same `CoefficientRing` but can have different
   * `StorageFormat`. The addition is optimized for each combination of `StorageFormat`.
   *
   * @param matrix The first matrix.
   * @param other The second matrix.
   ...
   */
  Sparse_matrix_z2z operator-() {
    Sparse_matrix_z2z res(this->_size.first, this->_size.second);
   ...
   *
   * Perform standard linear algebra multiplication. Matrices must have the same `CoefficientRing` but can have
   * different `StorageFormat`. The multiplication is optimized for each combination of `StorageFormat`.
   *
   * @param matrix The first matrix.
   * @param other The second matrix.
   * @return A reference to the modified matrix `matrix` containing the result.
   ...
   */
  friend Sparse_matrix_z2z<COLUMN>& operator*=(Sparse_matrix_z2z<COLUMN>& matrix,
                                               const Sparse_matrix_z2z<COLUMN>& other);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Matrix multiplication and assign: ROW *= ROW.
   */
  friend Sparse_matrix_z2z<ROW>& operator*=(Sparse_matrix_z2z<ROW>& matrix, const Sparse_matrix_z2z<ROW>& other);
...
public:
   ...
   */
  template <int _CTF>
  friend void set_coefficient(Sparse_matrix_z2z<_CTF>& matrix, size_t i, size_t j, const CGAL::Z2 d);
...
public:
   ...
   */
  template <int _CTF> friend CGAL::Z2 get_coefficient(const Sparse_matrix_z2z<_CTF>& matrix, size_t i, size_t j);
   ...
   */
  void set(size_t i, size_t j, Coefficient_ring value) { set_coefficient(i, j, value); }
   ...
   */
  Coefficient_ring get(size_t i, size_t j) const { return get_coefficient(i, j); }
   ...
   */
  Sparse_matrix_z2z& add(const Sparse_matrix_z2z& other) { return *this += other; }
   ...
   * \param j The column index.
   */
  void erase(size_t i, size_t j) { remove_coefficient(i, j); }
   ...
   *
   * \note For column-matrices, it is equivalent to `operator[]`, for row-matrices a traversal of the matrix is required
   * (in \f$\mathcal O(n)\f$).
   *
   * \warning The matrix will perform boundary check.
   *
   ...
   */
  friend Sparse_chain_z2z<COLUMN> get_column(const Sparse_matrix_z2z<COLUMN>& matrix, size_t index);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Gets a column from a ROW matrix.
   */
  friend Sparse_chain_z2z<COLUMN> get_column(const Sparse_matrix_z2z<ROW>& matrix, size_t index);
   ...
   *
   * \note For row-matrices, it is equivalent to `operator[]`, for column-matrices a traversal of the matrix is required
   * (in \f$\mathcal O(n)\f$).
   *
   * \warning The matrix will perform boundary check.
   *
   ...
   */
  friend Sparse_chain_z2z<ROW> get_row(const Sparse_matrix_z2z<COLUMN>& matrix, size_t index);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Gets a row from a ROW matrix.
   */
  friend Sparse_chain_z2z<ROW> get_row(const Sparse_matrix_z2z<ROW>& matrix, size_t index);
   ...
   */
  friend const Sparse_chain_z2z<COLUMN>& cget_column(const Sparse_matrix_z2z<COLUMN>& matrix, size_t index);
   ...
   */
  friend const Sparse_chain_z2z<ROW>& cget_row(const Sparse_matrix_z2z<ROW>& matrix, const size_t index);

  /** \relates Sparse_matrix_z2z
   *
   * \defgroup SetColumn Sets a column.
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief Set the value of the column at a given `index` from the matrix to `column` (whatever the `StorageFormat` of
   * the matrix).
   *
   * \note For column-matrices, it is equivalent to `operator[]` followed by an assignment, for row-matrices a traversal
   * of the matrix is required (in \f$\mathcal O(n)\f$).
   *
   * \warning The matrix will perform boundary check.
   *
   * @param matrix The  matrix.
   ...
   */
  friend void set_column(Sparse_matrix_z2z<COLUMN>& matrix, size_t index, const Sparse_chain_z2z<COLUMN>& column);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Sets a column in a ROW matrix.
   */
  friend void set_column(Sparse_matrix_z2z<ROW>& matrix, size_t index, const Sparse_chain_z2z<COLUMN>& column);
   ...
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief Set the value of the row at a given `index` from the matrix to `chain` (whatever the `StorageFormat` of the
   * matrix).
   *
   * \note For row-matrices, it is equivalent to `operator[]` followed by an assignment, for column-matrices a traversal
   * of the matrix is required (in \f$\mathcal O(n)\f$).
   *
   * \warning The matrix will perform boundary check.
   *
   ...
   */
  friend void set_row(Sparse_matrix_z2z<COLUMN>& matrix, size_t index, const Sparse_chain_z2z<ROW>& row);

  /** \relates Sparse_matrix_z2z
   *
   * \brief Sets a row in a ROW matrix.
   */
  friend void set_row(Sparse_matrix_z2z<ROW>& matrix, size_t index, const Sparse_chain_z2z<ROW>& row);
   ...
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief Build and return a new matrix by copying a `matrix` and removing  all chains with indices in the `indices`
   * vector (or at a given `index`). The result is thus a block of the initial matrix. When possible, for efficiency,
   * prefer `\=`.
   *
  ...
  Sparse_matrix_z2z operator/(const std::vector<size_t>& _indices) {
    Sparse_matrix_z2z res(*this);
    res /= _indices;
    return res;
  }

  /** \brief Removes the chain at a given `index` from a copy of the matrix. */
  Sparse_matrix_z2z operator/(size_t index) {
    Sparse_matrix_z2z res(*this);
    res /= index;
    return res;
  }
   ...
   * \ingroup PkgHDVFAlgorithmClasses
   * @brief Remove  all chains with indices in the `indices` vector (or at a given `index`). The result is thus a block
  ...
  Sparse_matrix_z2z& operator/=(const std::vector<size_t>& indices) {
  ...
  Sparse_matrix_z2z& operator/=(const size_t index) {
...
protected:
  ...
  Sparse_matrix_z2z& remove_column(size_t index) {
...
public:
   ...
   *
   * Removes column of index `index` whatever the `StorageFormat` of the matrix. For column matrices, it just comes to
   * the `\=` operator and for row matrices, it entails a traversal of the matrix.
   *
   * \param matrix Reference on the matrix to modify.
   ...
   */
  template <int _CTF> friend Sparse_matrix_z2z<_CTF>& remove_column(Sparse_matrix_z2z<_CTF>& matrix, size_t index);

protected:
  // Protected version of remove_row
  Sparse_matrix_z2z& remove_row(size_t index) {
    std::vector<size_t> tmp_id{index};
...
public:
   ...
   *
   * Removes row of index `index` whatever the `StorageFormat` of the matrix. For row matrices, it just comes to the
   * `\=` operator and for column matrices, it entails a traversal of the matrix.
   *
   * \param matrix Reference on the matrix to modify.
   * \param index The index to remove.
   *
   * \return A reference over the modified matrix.
   */
  template <int _CTF> friend Sparse_matrix_z2z<_CTF>& remove_row(Sparse_matrix_z2z<_CTF>& matrix, size_t index);
...
public:
   ...
   *
   * Swaps rows at indices `i` and `j` whatever the `StorageFormat` of the matrix. For row matrices, the operation runs
   * in constant time, while it entails a traversal of the matrix for column matrices.
   *
   * \param matrix Reference on the matrix to modify.
   ...
   * \return A reference over the modified matrix.
   */
  template <int _CTF> friend Sparse_matrix_z2z<_CTF>& swap_rows(Sparse_matrix_z2z<_CTF>& matrix, size_t i, size_t j);
...
public:
   ...
   *
   * Swaps columns at indices `i` and `j` whatever the `StorageFormat` of the matrix. For column matrices, the operation
   * runs in constant time, while it entails a traversal of the matrix for row matrices.
   *
   * \param matrix Reference on the matrix to modify.
   ...
   * \return A reference over the modified matrix.
   */
  template <int _CTF> friend Sparse_matrix_z2z<_CTF>& swap_columns(Sparse_matrix_z2z<_CTF>& matrix, size_t i, size_t j);

protected:
  // Protected version of remove_coefficient
  Sparse_matrix_z2z& remove_coefficient(size_t i, size_t j) {
    // OSM::COLUMN
    if(StorageFormat == OSM::COLUMN) {
...
public:
   ...
   */
  template <int _CTF>
  friend Sparse_matrix_z2z<_CTF>& remove_coefficient(Sparse_matrix_z2z<_CTF>& matrix, size_t i, size_t j);

  /**
   * \brief Iterator to the index of the first non null chain.
   *
   * Return an iterator to the index of the first non null chain (the iterator visits indices of non null chains along
   ...
   *
   * Return a reverse iiterator to the index of the last non null chain (the iterator visits indices of non null chains,
   * in decreading order, along the major dimension of the matrix).
   *
   ...
   *
   * \return A new matrix where the `StorageFormat` has been swapped between COLUMN and ROW and data chains have been
   * transposed.
   */
  Sparse_matrix_z2z<COLUMN + ROW - StorageFormat> transpose() const {
    Sparse_matrix_z2z<COLUMN + ROW - StorageFormat> transposed(this->_size.second, this->_size.first);
   ...
   */
  std::pair<size_t, size_t> dimensions() const { return this->_size; }
};
...
template <int _CTF> Sparse_matrix_z2z<_CTF> operator*(const Sparse_matrix_z2z<_CTF>& matrix, const CGAL::Z2& lambda) {
  Sparse_matrix_z2z<_CTF> newMatrix = matrix;
  newMatrix *= lambda;
...
Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& first, const Sparse_matrix_z2z<COLUMN>& second) {
  Sparse_matrix_z2z<COLUMN> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// ROW x COLUMN -> COLUMN
Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& first, const Sparse_matrix_z2z<COLUMN>& second) {
  Sparse_matrix_z2z<COLUMN> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// COLUMN x ROW -> COLUMN
Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& first, const Sparse_matrix_z2z<ROW>& second) {
  Sparse_matrix_z2z<COLUMN> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// ROW x ROW -> COLUMN
Sparse_matrix_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& first, const Sparse_matrix_z2z<ROW>& second) {
  Sparse_matrix_z2z<COLUMN> res(first._size.first, second._size.second);
...
// COLUMN matrix
Sparse_chain_z2z<COLUMN> operator*(const Sparse_matrix_z2z<COLUMN>& first, const Sparse_chain_z2z<COLUMN>& second) {
  // Perform col-col matrix multiplication with linear combination of columns.
  Sparse_chain_z2z<COLUMN> column(first._size.first);

  for(typename Sparse_chain_z2z<COLUMN>::const_iterator it = second.begin(); it != second.end(); ++it) {
...
// Matrix - column chain multiplication
// ROW matrix
Sparse_chain_z2z<COLUMN> operator*(const Sparse_matrix_z2z<ROW>& first, const Sparse_chain_z2z<COLUMN>& second) {
...
Sparse_chain_z2z<ROW> operator*(const Sparse_chain<_CT, ROW>& first, const Sparse_matrix_z2z<ROW>& second) {
  ...
  for(typename Sparse_chain_z2z<ROW>::const_iterator it = first.begin(); it != first.end(); ++it) {
...
// Row chain - matrix multiplication
// COLUMN matrix
Sparse_chain_z2z<ROW> operator*(const Sparse_chain_z2z<ROW>& first, const Sparse_matrix_z2z<COLUMN>& second) {
  // Perform row-col matrix multiplication with dots
  Sparse_chain_z2z<ROW> row(second._size.second);
...
// Matrix-matrix multiplication
// COLUMN x COLUMN -> ROW
Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<COLUMN>& first, const Sparse_matrix_z2z<COLUMN>& second) {
  Sparse_matrix_z2z<ROW> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// ROW x COLUMN -> ROW
Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<ROW>& first, const Sparse_matrix_z2z<COLUMN>& second) {
  Sparse_matrix_z2z<ROW> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// COLUMN x ROW -> ROW
Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<COLUMN>& first, const Sparse_matrix_z2z<ROW>& second) {
  Sparse_matrix_z2z<ROW> res(first._size.first, second._size.second);
...
// Matrix-matrix multiplication
// ROW x ROW -> ROW
Sparse_matrix_z2z<ROW> operator%(const Sparse_matrix_z2z<ROW>& first, const Sparse_matrix_z2z<ROW>& second) {
  Sparse_matrix_z2z<ROW> res(first._size.first, second._size.second);
...
const Sparse_chain_z2z<ROW>& cget_row(const Sparse_matrix_z2z<ROW>& matrix, const size_t index) {
...
void set_column(Sparse_matrix_z2z<COLUMN>& matrix, size_t index, const Sparse_chain_z2z<COLUMN>& chain) {
...
template <typename _CT> std::istream& read_matrix(Sparse_matrix_z2z<OSM::COLUMN>& M, std::istream& in) {
  ...
  size_t nrows, ncols;
  in >> nrows >> ncols;
  M = Sparse_matrix_z2z<OSM::COLUMN>(nrows, ncols);
...
template <typename _CT> std::istream& read_matrix(Sparse_matrix_z2z<OSM::ROW>& M, std::istream& in) {
  ...
  size_t nrows, ncols;
  in >> nrows >> ncols;
  M = Sparse_matrix_z2z<OSM::ROW>(nrows, ncols);
...
#endif // CGAL_OSM_SPARSE_MATRIX_Z2Z_H
